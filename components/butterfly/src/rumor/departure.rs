...
impl Departure {
    pub fn new<U>(member_id: U, rescinded: bool, incarnation: u64) -> Self
        where U: Into<String>
    {
        ...
        proto.set_rescinded(rescinded);
        proto.set_incarnation(incarnation);
...
impl Rumor for Departure {
    ...
    fn merge(&mut self, mut other: Departure) -> bool {
        if *self >= other {
            false
        } else {
...
mod tests {
    ...
    fn create_departure(member_id: &str) -> Departure {
        Departure::new(member_id, false, 0)
    }
    ...
    fn services_with_different_incarnations_are_not_equal() {
        let s1 = create_departure("mastodon");
        let mut s2 = create_departure("mastodon");
    ...
    #[test]
    fn departures_with_different_incarnations_are_not_equal_via_cmp() {
        let s1 = create_departure("adam");
        let mut s2 = create_departure("adam");
        s2.set_incarnation(1);
